...
export default {
  ...
  methods: {
    ...
    updateValue: function updateValue(_ref) {
      ...
      this.value = _Object$assign({}, this.value, _defineProperty({}, id, this.convertHalfWidth(value)));

      if (this.initialValue !== null) {
        this.checkifChanged(id);
      }
    },
    checkifChanged: function checkifChanged(id) {
      console.log(id, this.initialValue[id], this.value[id]);
      var stack = new Error().stack;
      console.error("I'm debugging this code.");
      this.$emit('hasChanges', this.initialValue[id] !== this.value[id]);
    },
    setInitialValue: function setInitialValue(initValue) {
      this.initialValue = initValue;
    },
    convertHalfWidth: function convertHalfWidth(str) {
